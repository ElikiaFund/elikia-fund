...
- `POST /onboarding/company` — `{ name, category, department, city }` (`category` is one of `Company::CATEGORIES`; `department` is one of `Company::DEPARTMENTS`, the Republic of Congo's 15 current departments — restructured from 12 in October 2024, this is currently the app's only real market). `city` is a **plain string, not a `Rule::in()` enum** — commune/arrondissement-level data is genuinely incomplete even in authoritative sources this soon after the reform, so the client offers `Company::DEPARTMENT_CAPITALS[$department]` as a one-tap default with a free-text "Autre" escape hatch, mirroring the existing `category`/`other_category` pattern rather than pretending to a false sense of completeness. 409 if the user already has one. Sets `users.onboarding_completed_at`. Also seeds this company's default product categories (see Products below).
...
- `Company::CATEGORIES`/`DEPARTMENTS` are each duplicated in 4 places (PHP const + DB enum + `mobile/src/constants/*` + `back-office/src/lib/*`) — update all 4 if either list ever changes.

## Products & services catalog (category, cost, stock)

A per-user catalog of sellable items (e.g. "Boisson", "Coupe de cheveux"), surfaced during onboarding (commerce/restauration/services companies only) and referenced from cash flow entries — now a real ledger of inventory value and margin, not just a name/price list, since that's the raw material a credit score (or a future lender) would actually want to see.

- **Categories are per-merchant, not a fixed platform catalog** — `product_categories` (`user_id`, `name`, `icon`, `color`) is a plain user-owned table (`Api\ProductCategoryController`, full CRUD at `/product-categories`), seeded with sector-appropriate defaults at onboarding (`ProductCategory::seedDefaultsFor()`, reusing `ProductFactory::CATALOG`'s existing vocabulary) but freely renamed/added/deleted after. Deleting a category `nullOnDelete()`s any products referencing it — it never cascade-deletes products.
- **`products`** (`Api\ProductController`, `/products*`): `name`, `category_id` (nullable FK), `sell_price` (renamed from `unit_price`), `cost_price` (a **running weighted-average**, recomputed on every restock — deliberately not per-batch/FIFO lot tracking, which would be real bookkeeping overhead a micro-business owner has no practical way to maintain by hand), `tracks_stock` (bool — off for pure services), `stock_quantity`, `low_stock_threshold`. `stock_quantity` is settable once at creation (the initial baseline) but **excluded from `PUT /products/{product}`** entirely — post-creation it only ever changes via `restock`/`adjust-stock`/a sale, each leaving an audit trail row.
- **`stock_movements`** (`product_id`, `user_id`, `type`: `restock`/`sale`/`adjustment`, signed `quantity_change`, `unit_cost` snapshot, nullable `transaction_id`) — the same "running balance + audit-trail movements" shape as `vaults`/`vault_movements`, the only other precedent for this pattern in the codebase. `App\Services\ProductStockService` owns every stock/cost side-effect, mirroring `VaultController`'s exact `DB::transaction()` increment/decrement-then-create-movement shape.
- **`POST /products/{product}/restock`** (`{ quantity, unit_cost, note, create_expense }`) — adds stock, recomputes `cost_price`, and — if `create_expense` (default **true**) — also creates a linked expense `Transaction` (`category: 'achat_stock'`) for the real cash that left the till, so "track every cent" extends to inventory purchases, not just sales.
- **`POST /products/{product}/adjust-stock`** (`{ quantity_change, note }`) — a manual correction (loss, breakage, recount); always blocking if it would take stock negative (unlike a sale, see below) since it's a deliberate correction happening in the moment, not something that already happened elsewhere.
- **Sale → stock link, and the online/offline enforcement asymmetry**: when an income `Transaction` carries a `product_id` (+ `quantity`), `ProductStockService::applyProductLink()` snapshots the product's current `cost_price` onto the transaction's own `unit_cost` (uniformly, whether or not the product tracks stock — so margin reporting works for services too) and, for stock-tracked products, decrements `stock_quantity` with a `sale` movement. **`TransactionController@store`** (online) enforces the stock limit — 422 "Stock insuffisant pour ce produit." — rolling back the whole transaction insert on failure. **`SyncController`** (offline batch sync) deliberately does **not** enforce it: an offline sale already physically happened by the time the server sees it, so it's accepted and `stock_quantity` is allowed to go negative as a visible signal to reconcile, rather than stranding the sale unsynced with no resolution path from the phone. Sync-retry idempotency (`$transaction->wasRecentlyCreated`) ensures a resent batch never double-decrements.
- `product_name`/`quantity` (the original free-text/int columns) are unchanged and still populated for entries with no catalog link — `product_id` is additive, not a replacement.
...
### Cycle payout — straight to the recipient's vault, not a real disbursement

`POST /groups/{group}/payout` (`{ cycle_period? }`, defaults to the current cycle) — owner-only (`abort_unless($group->owner_id === $request->user()->id, 403)`, same pattern as `designateRecipient`/`updateRecipientOrder`), and only once every eligible member (approved by the time the cycle closed, same rule `cycles()` already uses) has a `succeeded` contribution for it. All business rules live in `App\Services\TontinePayoutService::disburse()`, thrown as `App\Exceptions\TontinePayoutException` and turned into a 422 by the controller (same convention as `InsufficientStockException`):

- **Credits the recipient's vault, deliberately not a real mobile money disbursement.** A vault withdrawal is initiated by the account holder themselves, who types in a phone number at that moment (`VaultController::withdraw`) — a payout is triggered by the *owner*, on someone else's behalf, and there's no reliable mobile money number to send it to (a user's login `phone` isn't guaranteed to be one). Crediting the vault instead means the recipient can withdraw immediately through the existing withdraw flow if they want cash, or leave it as savings — either way it's real balance, not something that just leaves the system unrecorded.
- **The amount is the sum of `net_amount`** (post management-fee) across the cycle's `succeeded` contributions — the 3% fee stays with the platform, it's never disbursed.
- **Distinguished from a regular deposit on the ledger**: `vault_movements.type` gained a third enum value, `tontine_payout` (alongside `deposit`/`withdraw`), plus nullable `group_id`/`cycle_period` columns so a payout always traces back to the cycle that produced it. The vault tab's movement list and the PDF statement export (`buildVaultStatementHtml`) both render it as its own line — a "Versements tontine" total, separate from "Dépôts" — not lumped in.
- **Idempotent per cycle**: `group_cycle_recipients` (the existing per-cycle-recipient table) gained `paid_out_at` + `vault_movement_id`. `disburse()` resolves the recipient row, then re-fetches it with `lockForUpdate()` inside the transaction before checking `paid_out_at` — closes the race where two near-simultaneous payout requests for the same cycle could otherwise both pass the "not yet paid" check. A second call for an already-disbursed cycle 422s with "Ce cycle a déjà été versé."
- **The recipient must have already activated their vault.** No vault is auto-created on their behalf — `GET /vault` 404s purely on row existence (`VaultController::show`), which is what the mobile app's activation-vs-unlock routing depends on; auto-creating a vault with no PIN would make it appear "locked" with no way to ever unlock it. If the recipient hasn't activated theirs yet, the payout 422s with a clear message instead, and the owner can retry once they have.
- `show`/`store`/`join` responses also carry `current_cycle_all_paid` (computed in `withCycleStatus()`, same eligibility rule) so the mobile client can show the "Verser la cagnotte" action without a second round-trip, and `current_cycle_recipient.paid_out_at` so it can show "déjà versé" instead once it's done.
...
- Full data model migrations are in place (`transactions`, `vaults`, `vault_movements`, `groups`, `group_members`, `contributions`, `companies`, `products`, `product_categories`, `stock_movements`, `settings`, `scoring_criteria`, `notifications`, `yabeto_settings`) with models + relationships wired.
